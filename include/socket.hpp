...
/**
* @class net::Socket
* @desc  Socket类创建Berkeley风格socket
*/
    class Socket {
    private:
        union {
        ...
        * @desc    往sockfd上写数据
        *
        * @param   {callable}       _fn可调用对象,向sockfd指定文件描述符写入数据
        * @param   {string}         _msg中数据写向sockfd
        * @param   {parameter_pack} args为数据收发提供额外的控制
        * @returns {ssize_t}        成功时返回_msg的长度,失败时返回-1
        */
        ...
        * @desc 从sockfd上读数据
        *
        * @param {callable} _fn可调用对象,从sockfd上读数据
        * @param {string} _str 读取到的数据存入此处
        * @param {parameter_pack} args数据收取提供额外控制
        * @returns {ssize_t} 成功时返回非零值,失败是返回-1
        */
        template <typename Fn, typename... Args>
    ...
    public:
        ...
        */
                ...
                : sock_domain(_domain), sock_type(_type)
        {
            ...
            switch (sock_domain) {
                ...
                case Domain::UNIX: un.sun_family  = AF_UNIX; break;
        ...
        */
        Socket(Socket &&s)
        {
            ...
            switch (s.sock_domain) {
                ...
                case Domain::UNIX: un = s.un; break;
        ...
        */
        ...
        auto bind(F _fn) -> decltype(_fn(std::declval<AddrIPv4 &>()), void()) const
        {
            ...
            auto res = _fn(addr);
            if (res >= 1) {
                res = ::bind(sockfd, (sockaddr *) &addr, sizeof(addr));  //成功返回0,失败返回-1并设置errno
                res = (res == 0) ? 1 : res;
            }
